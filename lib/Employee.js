...
class Employee {
    ...
    getSalary() {
        return this.salary ? this.salary.toLocaleString('en-US', { style: 'currency', currency: 'USD' }) : "-";
    }
    ...
    getManager() {
        return this.manager ? this.manager : "-";
    }
            ...
            connection.query(query, [full_name], function (error, results, fields) {
                ...
                if (results.length > 0) {
                    ...
                    connection.query(query, [full_name], function (error, results, fields) {
                        if (error) throw error;
                        // Delete the employee
                        connection.query(delete_query, [full_name], function (error, results, fields) {
                            if (error) throw error;
                            resolve(results);
                        });
                    });
