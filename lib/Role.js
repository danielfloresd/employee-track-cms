...
class Role {
  ...
  static update(id, title, salary, department) {
    ...
    let query =
        'UPDATE roles SET title = ?, salary = ?, department_id = (SELECT id FROM departments WHERE name = ?) WHERE id = ?';
    // Query database
    let promise = new Promise((resolve, reject) => {
  ...
  static deleteById(id) {
    ...
    let promise = new Promise((resolve, reject) => {
      connection.query(
          'SELECT title FROM roles WHERE id = ?', [ id ], (err, res) => {
            if (err)
              throw err;
            // Call static delete method
            Role.delete(res[0].title).then((res) => { resolve(res); });
          });
    });
    // Return promise
    return promise;
  ...
  static delete(title) {
    ...
    let query =
        'SELECT * FROM employees LEFT JOIN roles ON employees.role_id = roles.id WHERE roles.title = ?';
    // Query database
    let promise = new Promise((resolve, reject) => {
  ...
  static getAll() {
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      let query =
          'SELECT r.id,r.title,r.salary,r.department_id,d.name  FROM roles r LEFT JOIN departments d ON r.department_id = d.id';
      connection.query(query, function(error, results, fields) {
        if (error)
          throw error;
        let roles = [];
        for (let i = 0; i < results.length; i++) {
          let role = new Role(results[i].id, results[i].title,
                              results[i].salary, results[i].name);
          roles.push(role);
        }
        resolve(roles);
      });
