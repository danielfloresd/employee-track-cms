// Create Department class with id, first_name, last_name, Department_id,
// manager_id Require db_connection.js
...
class Department {
  ...
  static deleteById(id) {
    ...
    let connection = DBConnection.getInstance().connection;
    // Find the department and call the static method delete(name)
    ...
    let promise = new Promise((resolve, reject) => {
      connection.query(query, [ id ], (err, res) => {
        if (err)
          throw err;
        Department.delete(res[0].name).then((res) => { resolve(res); });
      });
    });
    // Return promise
    return promise;
  ...
  static delete(name) {
    ...
    let query =
        'UPDATE employees SET role_id = NULL WHERE role_id IN (SELECT id FROM roles WHERE department_id = (SELECT id FROM departments WHERE name = ?))';
    // Query database
    let promise = new Promise((resolve, reject) => {
      connection.query(query, [ name ], (err, res) => {
        ...
        // Delete the role
        let query =
            'DELETE FROM roles WHERE department_id = (SELECT id FROM departments WHERE name = ?)';
        connection.query(query, [ name ], (err, res) => {
          if (err)
            throw err;
  ...
  static create(name) {
    ...
    let promise = new Promise((resolve, reject) => {
      connection.query('INSERT INTO departments SET ?', {name : name},
                       (err, res) => {
                         if (err)
                           throw err;
                         resolve(res);
