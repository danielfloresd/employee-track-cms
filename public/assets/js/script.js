...
const loadRoles = () => {
  ...
  getRoles().then(data => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      let salary = $('<td>').text(role.salary.toLocaleString('en-US', { style: 'currency', currency: 'USD' }));
  ...
  getDepartments()
    .then((data) => {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        let num_emplpoyees = EMPLOYEES.filter(e => e.department == data[i].name).length;
        let budget = EMPLOYEES.filter(e => e.department == data[i].name).reduce((a, b) => a + b.salary, 0);
        let row = $("<tr>");
        row.append($("<td>").text(data[i].name));
        row.append($("<td>").text(num_emplpoyees));
        row.append($("<td>").text(budget.toLocaleString('en-US', { style: 'currency', currency: 'USD' })));
...
const selectDepartment = (department) => {
  ...
  let employees = !department ? EMPLOYEES : EMPLOYEES.filter(employee => employee.department === department.name);
  ...
  for (let i = 0; i < employees.length; i++) {
    ...
    let salary = employee.salary ? employee.salary.toLocaleString('en-US', { style: 'currency', currency: 'USD' }) : "";
    td = $('<td>')
      .text(salary);
    tr.append(td);
...
const createEployeeCard = (employee) => {
  ...
  let cardTitle = $('<h3>').addClass("card-title").text(employee.first_name + " " + employee.last_name);
  ...
  let labelManager = $('<p>')
    ...
    .attr("style", "margin-top: 2px; width: 100px;")
    .text("Manager");
  let employee_options = EMPLOYEES.map(employee => employee.first_name + " " + employee.last_name);
...
const createNewEmployeeCard = (employee) => {
  ...
  let saveButton = $('<button>')
    ...
    .click(function () {
      ...
      let manager_index = EMPLOYEES.findIndex(e => e.first_name + " " + e.last_name === manager);
...
const createRoleCard = (role) => {
  ...
  let labelDepartment = $('<p>')
    ...
    .text("Department");
  // let cardDepartmentDropdown = createDropDown(department);
  let cardDepartmentDropdown = createDropDown("department", department, DEPARTMENTS.map(department => department.name), role);
...
const createNewRoleCard = (role) => {
  ...
  let labelDepartment = $('<p>')
    .attr("style", "margin-top: 10px; width: 100px;")
    .text("Department");
  let cardDepartmentDropdown = createDropDown("department", "Select", DEPARTMENTS.map(department => department.name), role);
  let labelSalary = $('<p>')
    .attr("style", "margin-top: 10px; width: 100px;")
    .text("Salary");
  let cardSalaryInput = $('<input>')
