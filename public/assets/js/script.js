...
const loadRoles = () => {
  ...
  getRoles().then((data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      let salary = $("<td>").text(role.salary.toLocaleString("en-US", {
        style : "currency",
        currency : "USD",
      }));
...
const selectDepartment = (department) => {
  ...
  for (let i = 0; i < employees.length; i++) {
    ...
    let salary = employee.salary ? employee.salary.toLocaleString("en-US", {
      style : "currency",
      currency : "USD",
    })
                                 : "";
...
const createEmployeeCard = (employee) => {
  ...
  let labelManager = $("<p>")
                         ...
                         .attr("style", "margin-top: 2px; width: 100px;")
                         .text("Manager");
  let employee_options = EMPLOYEES.map((employee) => employee.first_name + " " +
                                                     employee.last_name);
...
const createNewEmployeeCard = (employee) => {
  ...
  let cardRoleDropdown =
      createDropDown("title", "Select", role_options, employee);
  ...
  let employee_options = EMPLOYEES.map((employee) => employee.first_name + " " +
                                                     employee.last_name);
  let cardManagerDropdown =
      createDropDown("manager", "Select", employee_options, employee);
  let cardButtons = $("<div>").addClass("btn-group-vertical");
...
const createNewRoleCard = (role) => {
  ...
  let labelTitle =
      $("<p>").attr("style", "margin-top: 10px; width: 100px;").text("Title");
  ...
  let cardDepartmentDropdown =
      createDropDown("department", "Select",
                     DEPARTMENTS.map((department) => department.name), role);
  let labelSalary =
      $("<p>").attr("style", "margin-top: 10px; width: 100px;").text("Salary");
