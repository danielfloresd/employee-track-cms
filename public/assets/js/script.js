...
const getDatabaseURL =
    () => {
      return fetch('/api/database_url')
          .then(response => response.json())
          .then(data => { console.log("DATABASE_URL", data); });
    }
...
const deleteEmployee =
    (employee) => {
      ...
      let result =
          confirm("Are you sure you want to delete " + employee.first_name +
                  " " + employee.last_name + "?");
      if (result) {
...
const saveEmployee = (employee) => { fetchAPI(employee, 'PUT', 'employees'); }

const newEmployee = (employee) => { fetchAPI(employee, 'POST', 'employees'); }

const saveRole = (role) => { fetchAPI(role, 'PUT', 'roles'); }

const deleteRole = (role) => { fetchAPI(role, 'DELETE', 'roles'); }

const newRole = (role) => { fetchAPI(role, 'POST', 'roles'); }

const saveDepartment =
    (department) => { fetchAPI(department, 'PUT', 'departments'); }

const deleteDepartment =
    (department) => { fetchAPI(department, 'DELETE', 'departments'); }

// Create newDepartment function
const newDepartment =
    (department) => { fetchAPI(department, 'POST', 'departments'); }
    ...
    () => {
      ...
      getRoles().then(data => {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          let salary = $('<td>').text(role.salary.toLocaleString(
          ...
          row.dblclick(function() { editRole(role); });
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          let li = $('<li>')
                       .append($("<button>")
                                   ...
                                   .text(data[i].name))
                       .attr("id", data[i].id)
                       .click(function() { selectDepartment(data[i]); });
          $("#employees").append(li);

          // Add row to departments table
          let num_emplpoyees =
              EMPLOYEES.filter(e => e.department == data[i].name).length;
          let budget = EMPLOYEES.filter(e => e.department == data[i].name)
                           .reduce((a, b) => a + b.salary, 0);
          let row = $("<tr>");
          row.append($("<td>").text(data[i].name));
          row.append($("<td>").text(num_emplpoyees));
          row.append($("<td>").text(budget.toLocaleString(
              'en-US', {style : 'currency', currency : 'USD'})));

          // Add double click event to row
          row.dblclick(function() { editDepartment(data[i]); });
...
const initDepartments =
    () => {
      ...
      let li =
          $('<li>')
              .append(
                  $("<button>").addClass("btn btn-info btn-block").text("All"))
              .attr("id", "all")
              .click(function() { selectDepartment(); });
      $("#employees").append(li);
...
const selectDepartment =
    (department) => {
      ...
      let employees = !department
                          ? EMPLOYEES
                          : EMPLOYEES.filter(employee => employee.department ===
                                                         department.name);
      ...
      for (let i = 0; i < employees.length; i++) {
        ...
        // tr.append(tdActions);
        // Add double click event to row
        tr.dblclick(function() { editEmployee(employee); });
...
const createEployeeCard =
    (employee) => {
      ...
      let cardRoleDropdown =
          createDropDown("title", role, role_options, employee);
      let labelDepartment = $('<p>')
                                // .addClass("card-subtitle mb-2 text-muted")
                                .attr("style", "margin-top: 2px; width: 100px;")
                                .text("Department");
      // let cardDepartmentDropdown = createDropDown(department);
      let cardDepartmentDropdown =
          $('<p>')
              // .addClass("card-subtitle mb-2 text-muted")
              .attr("style", "margin-top: 2px; width: 100px;")
              .text(department);
      ...
      let cardManagerDropdown =
          createDropDown("manager", manager, employee_options, employee);
...
const createNewEmployeeCard =
    (employee) => {
      ...
      cardButtons.append(cardNameGroup, cardLastNameGroup, cardRoleGroup,
                         cardManagerGroup);
      // Add save and cancel buttons
      let saveButton =
          $('<button>')
...
const createDropDown =
    (key, label, options, employee) => {
      ...
      select.change(function() { employee[key] = $(this).val(); });
...
const createRoleCard =
    (role) => {
      ...
      // let cardDepartmentDropdown = createDropDown(department);
      let cardDepartmentDropdown =
          createDropDown("department", department,
                         DEPARTMENTS.map(department => department.name), role);
      ...
      cardBody.append(cardIcon, cardTitle, cardSalary, cardSalaryInput,
                      cardBreak);
      cardDepartmentGroup.append(labelDepartment, cardDepartmentDropdown);
    ...
    (role) => {
      ...
      let labelDepartment =
          $('<p>')
              ...
              .text("Department");
      let cardDepartmentDropdown =
          createDropDown("department", "Select",
                         DEPARTMENTS.map(department => department.name), role);
      let labelSalary = $('<p>')
                            .attr("style", "margin-top: 10px; width: 100px;")
                            .text("Salary");
